...
export default function CreateBlog() {
  ...
  useEffect(() => {
    ...
    document.addEventListener("mousedown", handleClickOutside); // Lắng nghe click toàn cục
    return () => {
      document.removeEventListener("mousedown", handleClickOutside); // Dọn sự kiện khi component unmount
    };
  }, []);
            ...
            <div className="relative mt-2">
              ...
              <div className="flex items-center gap-2 mt-2" ref={dropdownRef} >
                ...
                <input
                  ...
                  onFocus={() => setShowDropdown(true)} // Hiển thị danh sách khi click vào input
                  placeholder="Nhập hoặc chọn tiện ích"
                  className="w-full p-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                <button
                  type="button"
                  className="text-white bg-green-600 hover:bg-green-700 font-bold py-2 px-4 rounded-sm"
                  onClick={handleAddUtility}
                >
                  +
                </button>
                {showDropdown && (
                  <ul className="absolute top-full left-0 mt-1 w-full bg-white border border-gray-300 rounded shadow-lg z-10">
                    {utilities.map((utility, index) => (
                      <li
                        key={index}
                        className="p-2 hover:bg-gray-100 cursor-pointer"
                        onClick={() => {
                          setNewUtilityName(utility.name); // Gán giá trị vào input
                          setShowDropdown(false); // Ẩn dropdown
                        }}
                      >
                        {utility.name}
