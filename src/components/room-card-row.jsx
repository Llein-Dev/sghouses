...
export function RoomCardRowComponent({
    product
}) {
    console.log(product);
                ...
                <div className="flex flex-col justify-between w-full sm:w-2/3">
                    <CardContent className="p-4 space-y-2">
                        <CardTitle className="flex justify-between">
                            <div>
                                <Link href={`/building/${product?.slug_toa_nha}`}>
                                    <h3 className="text-lg font-semibold mb-2">{product?.ten_phong}</h3>
                                </Link>
                                <Link href={`/building/${product?.slug_toa_nha}`}>
                                    <h6 className="text-sm font-normal mb-2"><span className="underline">Tòa nhà:</span> {product?.ten_toa_nha}</h6>
                                </Link>
                            </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {/* Area (Khu vực) */}
                            {product?.ten_khu_vuc && (
                                <Badge variant="outline" className="bg-blue-100 text-blue-700">{product?.ten_khu_vuc}</Badge>
                            )}

                            {/* Room Size (Diện tích) */}
                            {product?.dien_tich && (
                                <Badge variant="outline" className="bg-green-100 text-green-700">{product?.dien_tich} m²</Badge>
                            )}

                            {/* Electricity Rate (Đơn giá điện) */}
                            {product?.don_gia_dien && (
                                <Badge variant="outline" className="bg-yellow-100 text-yellow-700">
                                    <span className="mr-1"></span>Điện: {product?.don_gia_dien} VND/kWh
                                </Badge>
                            )}
                            ...
                            {product?.don_gia_nuoc && (
                                <Badge variant="outline" className="bg-cyan-100 text-cyan-700">
                                    <span className="mr-1"></span>Nước {product?.don_gia_nuoc} VND/m³
                                </Badge>
                            )}
                            ...
                            {product?.phi_dich_vu && (
                                <Badge variant="outline" className="bg-pink-100 text-pink-700">
                                    <span className="mr-1"></span>Dịch vụ: {product?.phi_dich_vu} VND
                                </Badge>
                            )}
                            ...
                            {product?.tien_ich && product?.tien_ich !== "Không" && (
                                <div className="flex flex-wrap gap-2">
                                    {product.tien_ich.split(";").map((item, index) => (
                                        <Badge key={index} variant="outline" className="bg-teal-100 text-teal-700">
                                            {item.trim()}
                                        </Badge>
                                    ))}
                                </div>
                            ...
                            {product?.tien_xe_may && (
                                <Badge variant="outline" className="bg-orange-100 text-orange-700">
                                    <span className="mr-1"></span>Xe: {product?.tien_xe_may} VND
                                </Badge>
                            )}
                            ...
                            {product?.gac_lung && product?.gac_lung !== "Không" && (
                                <Badge variant="outline" className="bg-indigo-100 text-indigo-700">
                                    <span className="mr-1"></span>Gác lửng: {product?.gac_lung}
                                </Badge>
                            )}
                        </div>
